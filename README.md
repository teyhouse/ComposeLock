...
Containers that exit with code 0, such as one-shot migration or init jobs, count as completed rather than failed, including the healthcheck they last reported: a finished job whose final probe was unhealthy does not fail the watch. Services removed from the Compose file are removed from the stack. A container that happens to be `restarting` passes, both at the final poll and at the pre-flight gate, as long as its restart count is still within tolerance, so ordinary `restart: always` churn neither rolls back a healthy deploy nor blocks the next one.
...
Teardown and checkout restore deliberately run to completion through a SIGINT or SIGTERM, because a half-removed stack is worse than a slow exit. Shutdown during a teardown is therefore bounded by `docker_timeout_seconds` per stack, capped at two minutes each, plus 30 seconds for the checkout restore. All three modes share that contract: `sync` and `poll` run the reconcile to the end, and `webhook` stops accepting triggers and then waits for the in-flight reconcile before the process exits.
