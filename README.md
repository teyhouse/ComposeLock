...
Teardown and checkout restore deliberately run to completion through a SIGINT or SIGTERM, because a half-removed stack is worse than a slow exit. Shutdown during a teardown is therefore bounded by `docker_timeout_seconds` per stack, capped at two minutes each, plus 30 seconds for the checkout restore.
