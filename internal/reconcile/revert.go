...
func doRevert(ctx context.Context, deps Deps, st *state.State, failedCommit, reason string, start time.Time) Result {
	...
	if err := deps.Git.Checkout(ctx, target); err != nil {
		return degrade(deps, st, failedCommit, target, fmt.Errorf("checking out rollback target %s: %w", target, err), start)
	}

	project, err := deps.Compose.LoadProject(ctx, cfg.ComposeFile, cfg.ProjectName)
	if err != nil {
		return degrade(deps, st, failedCommit, target, fmt.Errorf("loading project for revert: %w", err), start)
	}

	if err := deps.Compose.Up(ctx, project); err != nil {
		return degrade(deps, st, failedCommit, target, fmt.Errorf("compose up during revert: %w", err), start)
	}
	...
	revertErr := fmt.Errorf("applied %s failed health watch (%s), reverted to %s — healthy again", shortCommit(failedCommit), reason, shortCommit(target))
