...
| `notify_webhook.url`            | (disabled)            | URL that receives every notification as a JSON `POST`, see [Generic webhook](notifications.md#generic-webhook) |
| `notify_webhook.headers`        | (none)                | Extra request headers for `notify_webhook.url`, e.g. `{"Authorization": "Bearer ..."}` |
